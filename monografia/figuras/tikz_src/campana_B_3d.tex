...
% campo B: abanico en plano y-z
\foreach \t in {0,20,40,60,80}{
  \draw[Bfan] \PT{0}{0}{0} -- \PT{0}{\L*sin(\t)}{\L*cos(\t)};
}
\draw[Bvec] \PT{0}{0}{0} -- \PT{0}{\L*sin(52)}{\L*cos(52)};
\node[red!75!black] at \PT{0}{\L*sin(52)+0.2}{\L*cos(52)+0.1} {$\mathbf B$};
\node[font=\small] at \PT{0}{0.55}{1.45} {$\theta$};

% titulo + nota
\node[font=\bfseries] at \PT{0}{1.5}{3.5} {Campaña B: orientación $y$--$z$};
\node[font=\footnotesize,align=center] at \PT{1.2}{1.6}{-1.7}
  {$\mathbf B$ rota en el plano $y$--$z$; como $\hat y\parallel\mathbf k$,\\ $B_y$ ejerce \textbf{tensión} $\propto B_y^2$};
\end{tikzpicture}
\end{document}
