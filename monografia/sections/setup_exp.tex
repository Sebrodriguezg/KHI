...
El perfil de densidad sigue la configuración canónica Mizuno-like: \sebnota{citar} el flujo central (interior, $|x|<0.5$) presenta densidad baja, mientras que las regiones externas mantienen una densidad mayor. La transición se construye mediante una ventana suave a partir de funciones tangentes hiperbólicas:
\begin{equation}
\begin{aligned}
...
\end{equation}
Al variar $B_0$ dos órdenes de magnitud, la presión magnética $P_{\rm mag}=B^2/2$, el parámetro de plasma $\beta=2P_0/B^2\approx0.99/B_0^2$ y la magnetización $\sigma_m\approx B^2/(\rho h)\approx0.404\,B_0^2$ (con $\rho h=5$) recorren regímenes desde dominado por presión ($\beta\gg1$) hasta dominado por campo ($\beta<1$); se espera que el aumento de $B_0$ refuerce la tensión magnética y estabilice la KHI (Cuadro~\ref{tab:campana_A}).\claudenota{REVISIÓN GLOBAL (Claude): dos puntos. (1) ``el aumento de $B_0$ refuerza la \emph{tensión}'' contradice el cap.~6 (corregido): en orientación Mizuno $M_{A,\parallel}$ es super-Alfvénico siempre $\Rightarrow$ la supresión es por \emph{presión/magnetización} ($\beta\propto B_0^{-2}$), no tensión. Reformular. (2) Esta tabla lista 5 casos ($B_0=0.25$–$2.0$) pero los débiles ($0.25,0.5$) crashearon y NO están en resultados (cap.~6 solo A3/A4/A5); marcar cuáles se analizan realmente. Igual la Campaña B incluye $\theta=15°$, excluido de los datos limpios.}
...
\end{equation}
con $\theta\in\{0,\,5.7,\,15,\,30,\,45,\,60,\,90\}^\circ$.\sebmota{Mencionar que se intento pero limitados numericamente no se logro la evolicion de todos los caso 15 muere} El caso Mizuno original corresponde a $\theta_{\rm Mizuno}=\arctan\!\sqrt{0.02/2.0}\approx5.7^\circ$; el extremo $\theta=0^\circ$ es campo guía puro ($\mathbf B=B_{\rm tot}\hat z$) y $\theta=90^\circ$ es campo enteramente en $\hat y$ (Cuadro~\ref{tab:campana_B}). Esta familia aísla si la pequeña componente transversal $B_y$ del setup Mizuno tiene un efecto dinámico apreciable.
...
\end{equation}
A diferencia de la Campaña~B, la rotación genera aquí una componente $B_x$ \emph{paralela} al flujo de cizalla, que puede estabilizar de forma más directa las perturbaciones mediante tensión magnética a lo largo de la dirección del corte (Cuadro~\ref{tab:campana_C}).\claudenota{ERROR FÍSICO (revisión Claude, PRIORIDAD — incoherencia con cap.~6): $B_x$ NO es paralela al flujo. El flujo de cizalla va en $\hat y$ ($V_y$) y el modo $\mathbf k$ también en $\hat y$; $B_x$ es \emph{perpendicular} al flujo y normal a la interfaz ($\mathbf k\cdot\mathbf B\propto B_x\cdot 0=0$) $\Rightarrow$ NO ejerce tensión. El cap.~6 ya describe (correctamente) la Campaña C como ``sin tensión''. CORREGIR este párrafo: la tensión la da $B_y$ ($\parallel\mathbf k$, Campaña B), no $B_x$. La variación con $\theta$ en C es por presión/reconexión, no tensión.}
