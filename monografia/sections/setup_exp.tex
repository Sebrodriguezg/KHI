...
\begin{equation}
    \rho(x) = \rho_{lo} + (\rho_{up} - \rho_{lo})\left[\,1 - W(x)\,\right],\qquad
...
%   El entorno {subfigure} requiere el paquete `subcaption`, NO cargado en el
%   preámbulo del proyecto. Esto generó:
%       ! LaTeX Error: Environment subfigure undefined.
%       ! Undefined control sequence ... \subref{...}
%
% SOLUCIONES POSIBLES (elegir UNA):
%
%   (A) Cargar el paquete en el preámbulo principal (grad.tex), una sola línea:
%         \usepackage{subcaption}
%       Y mantener la figura compuesta original con \begin{subfigure}{...}.
%
%   (B) Reemplazar la figura compuesta por dos figuras independientes usando
%       \minipage (entorno nativo de LaTeX, sin dependencias externas).
%       Esta es la opción más portable y NO requiere tocar el preámbulo.
%
% A continuación se entrega el bloque SOLO de la figura, ya reescrito con la
% opción (B). Reemplazar el bloque \begin{figure}...\end{figure} actual en
% sections/setup_exp.tex (líneas ~71-92 según el log) por el siguiente:
% ============================================================================
...
% ============================================================================
% OBSERVACIONES ADICIONALES SOBRE EL LOG (no críticas para esta sección):
%
%   1. "Label `sec:setup_experimental' multiply defined":
%      La sección antigua y la nueva coexisten en el proyecto. Eliminar la
%      versión obsoleta de \section{Configuración Experimental} de cualquier
%      otro archivo .tex incluido en grad.tex para evitar etiquetas duplicadas.
%
%   2. "Reference `sec:RRMHD_Conservativa' / `sec:Max-aumentado' /
%       `sec:Ohm_Relativista' on page X undefined":
%      Las referencias cruzadas al marco teórico requieren un segundo pase de
%      pdflatex (más bibtex/biber si aplica) para resolverse. Ejecutar:
%         pdflatex grad.tex
%         biber   grad      (o bibtex grad)
%         pdflatex grad.tex
%         pdflatex grad.tex
%      Tras la segunda compilación, dichas warnings deben desaparecer si las
%      etiquetas existen efectivamente en los capítulos correspondientes.
%
%   3. "Citation 'Mizuno2013' undefined":
%      Verificar que la entrada @article{Mizuno2013, ...} esté declarada en el
%      archivo .bib del proyecto y que biber/bibtex se ejecute en el ciclo.
% ============================================================================
...
\begin{equation}
    \sigma \in \{\,\sigma_1,\, \sigma_2,\, \ldots,\, \sigma_N\,\}
    % [REEMPLAZAR CON LOS VALORES FINALES, ej. \{10^0, 10^1, 10^2, 10^3, 10^4, 10^5, 10^6\}]
\end{equation}
