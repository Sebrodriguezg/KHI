...
\begin{figure}[H]
    ...
    \begin{minipage}{0.95\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figuras/velocidad_inicial.pdf}
        % \includegraphics[width=\linewidth]{figuras/frame_0001_campoV.png}

        \vspace{0.3em}
        \small (a) Campo de velocidad: líneas de corriente sobre el mapa de $|\mathbf{V}(x,y)|$. Se aprecian las dos capas de cizalla en $x = \pm 0.5$ y la modulación senoidal de la perturbación (Ec.~\ref{eq:Vx_pert}), semilla del modo inestable.
    \end{minipage}
    ...
    \begin{minipage}{0.95\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figuras/rho_inicial.pdf}
        % \includegraphics[width=\linewidth]{figuras/frame_0001_rho.png}

        \vspace{0.3em}
        \small (b) Densidad $\rho(x,y)$: flujo central diluido ($\rho \simeq \rho_{lo}$, $|x| < 0.5$) embebido en un ambiente denso ($\rho \simeq \rho_{up}$), según la Ec.~\ref{eq:rho_profile}. En $t=0$ las interfaces son rectas; la semilla reside en la velocidad (panel~a).
    \end{minipage}

    \caption{Estado inicial del experimento KHI (Test~35A, $t=0$): campo de velocidad~(a) y densidad~(b). Junto con el campo magnético uniforme de fondo (Ec.~\ref{eq:B_field_init}), constituyen el estado base del barrido en la conductividad $\sigma$.}
...
\begin{equation}
   ...
   \label{eq:Vy_profile}
\end{equation}
donde $v_{sh} = 0.5\, c$ (factor de Lorentz relativo moderado $W_{\text{rel}} \simeq 1.29$) y $a_{kh} = 0.05$ define el ancho característico de la capa de cizalladura, garantizando que la escala dominante $k_{kh}^{-1}\!\gtrsim\!a_{kh}$ permita una resolución espectral adecuada del modo lineal inestable.
...
\begin{equation}
\begin{aligned}
    \rho(x) &= \rho_{lo} + (\rho_{up} - \rho_{lo})\left[\,1 - \mathcal{W}(x)\,\right],\\[2pt]
    \mathcal{W}(x) &= \tfrac{1}{2}\!\left[1 + \tanh\!\left(\tfrac{x+0.5}{a_{kh}}\right)\right]\!\cdot\!\tfrac{1}{2}\!\left[1 - \tanh\!\left(\tfrac{x-0.5}{a_{kh}}\right)\right]
\end{aligned}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.48\textwidth]{figuras/campana_A_3d}\hfill
\includegraphics[width=0.48\textwidth]{figuras/campana_B_3d}

\vspace{0.6em}
\includegraphics[width=0.48\textwidth]{figuras/campana_C_3d}
...
\end{equation}
A diferencia de la Campaña~B, la rotación geométrica en este caso genera una componente $B_x$ que es \emph{perpendicular} al flujo de cizalla y normal a la interfaz. Dado que el vector de onda $\mathbf{k}$ de la inestabilidad se orienta a lo largo de la dirección longitudinal ($\hat{y}$), esta configuración asegura de forma analítica que la componente transversal no ejerza tensión magnética directa sobre la perturbación ($\mathbf{k}\cdot\mathbf{B} = 0$). Por lo tanto, la dinámica evaluada al variar $\theta$ en esta campaña aísla los efectos topológicos derivados de la incidencia angular de $B_x$ sobre la interfaz, propiciando el estudio de procesos de reconexión magnética en un escenario libre de soporte por tensión (Tabla~\ref{tab:campana_C}).
...
\begin{itemize}
    ...
    \item \textbf{Máxima densidad de corriente:} $J_{\max}(t)$, correspondiente al valor local máximo del vector $\mathbf{J} = \nabla \times \mathbf{B}$ en el dominio computacional. Resulta fundamental para identificar el adelgazamiento de las capas de corriente y señalar los instantes precisos de reconexión magnética.
