...
\subsection{Campaña de variación del campo magnético (objetivo 4)}
\label{subsec:campana_B}

\primerparrafo El cuarto objetivo ---evaluar cómo la \emph{intensidad} y la \emph{orientación} del campo magnético externo modifican la estabilización o amplificación de la KHI--- requiere una campaña dedicada que varíe $\mathbf{B}$ sobre el mismo montaje base (Sección~\ref{subsec:condiciones_iniciales}), manteniendo \emph{fija} la conductividad para aislar el efecto magnético del resistivo. La campaña se organiza en tres familias complementarias, todas con $|\mathbf B|=\sqrt{2.02}\,B_0$ en el caso de referencia ($B_0=1$).
