...
\begin{equation}
V_A=\sqrt{\frac{B^2}{\rho h+B^2}}=0.5364,\quad
V_{A,\parallel}=\sqrt{\frac{B_y^2}{\rho h+B^2}}=0.0534,\quad
...
\primerparrafo Los Mach simples son $M_s=\vsh/\mathcal{C}_s=0.968$, $M_{A,\parallel}=\vsh/V_{A,\parallel}=9.37$ y $M_f=\vsh/v_{f\perp}=0.723$. La teoría lineal relativista \parencite{osmanov2008,bodo-2004,chow2023} no usa el cociente simple sino el Mach \emph{relativista} de Königl, que pondera cada velocidad por su factor de Lorentz:
\begin{equation}
...
\label{ap:eq:leeslin}
\end{equation}
El término $(U-c)^2/c_s^2$ es la corrección compresible: cuando $|U-c|\to c_s$ el modo se vuelve sónico y el crecimiento se suprime (germen del umbral de Landau). Multiplicando por $(U-c)$ se obtiene un operador \textbf{cúbico en $c$} que se linealiza en forma \emph{companion} y se resuelve por \texttt{scipy.linalg.eig}. \textbf{Validación:} en el límite incompresible ($c_s\to\infty$) reproduce $\what_{i,\max}\to0.1897$ (Michalke). Con $c_s\to v_{f\perp}$ ($c_s^{\rm norm}=1.382$) en el modo $ka=0.314$:
\begin{equation}
...
\begin{equation}
\frac{\gamma_1^2(\omega-U_1k)^2}{a_1}+\frac{\gamma_2^2(\omega-U_2k)^2}{a_2}=0,
\qquad a_i^2=\gamma_i^2\Big[(k-U_i\omega)^2-\frac{(\omega-U_ik)^2}{c_s^2}\Big] .
\label{ap:eq:DR}
\end{equation}
...
\begin{equation}
\scrit=\frac{\sum_i \sigma_i/\delta\sigma_i^2}{\sum_i 1/\delta\sigma_i^2}=2861,
\qquad
...
\primerparrafo La Tabla~\ref{ap:tab:sintesis} sintetiza la verificación independiente. El \textit{Cueva}, en \texttt{TEST=35}, produce una KHI cuya tasa de crecimiento lineal es \textbf{consistente con la predicción de la teoría lineal RMHD compresible} (dentro del $8\%$) y cae \textbf{dentro de la región de inestabilidad relativista} ($M_{re}=0.60<\sqrt2$, sin extrapolación). El déficit respecto al techo hidrodinámico no es un error del código: es el efecto físico de compresibilidad relativista sostenido por la presión del campo guía $B_z$. La resistividad finita del marco RRMHD es además lo que vuelve bien planteado el problema \parencite{lecoanet2016}. El valor preciso de la asíntota ideal $C+\gamma_0$ es, no obstante, una \emph{extrapolación respaldada por consistencia}, no un plateau medido; su confirmación cerrada requiere las simulaciones a $\Rmst\gtrsim500$.
