...
\primerparrafo El segundo conjunto de ecuaciones describe la dinámica del campo en presencia de fuentes materiales. Para ello, es necesario introducir el cuadrivector densidad de corriente, $J^\mu$, el cual unifica la densidad de carga eléctrica $\rho_e$ y el vector de densidad de corriente tridimensional $\mathbf{J}$ en un solo objeto geométrico:
\begin{equation}
    J^\mu = (\rho_e, \mathbf{J})
\end{equation}
...
\end{equation}
donde hemos introducido los escalares invariantes $E^2 = E^\mu E_\mu$ y $B^2 = B^\mu B_\mu$, que representan la densidad de energía de cada campo, y el cuadrivector covariante de Poynting:%
\revnota{el proyector $h^{\mu\nu}$ se usa aquí pero se define mucho después (Sec.~\ref{sec:Ohm_Relativista}); además colisiona con la entalpía específica $h$. Definir el proyector antes y/o distinguir notación.}
\begin{equation}
    S^\mu = \epsilon^{\mu\nu\alpha\beta} u_\nu E_\alpha B_\beta
\end{equation}
...
\begin{align}
    ...
    \partial_t \Phi + \nabla \cdot \mathbf{E} &= \rho_e - \kappa_E \Phi \label{eq:GLM_3D_Phi} \\
...
\revnota{de aquí en adelante varias ecuaciones clave (descomposición de la corriente, ley de Ohm) van en \texttt{\$\$...\$\$} sin numerar; usar \texttt{equation} y numerarlas por consistencia con el resto del capítulo.}%
...
Esta separación define unívocamente un escalar invariante de Lorentz y un vector puramente espacial en el marco propio del fluido. \revnota{el símbolo $\rho_e$ se usa antes (Ec.~\ref{eq:Maxwell_Inhomogeneas} y sistema GLM 3+1) como carga del \emph{laboratorio}, pero aquí pasa a ser la carga \emph{propia}; y la misma ec.\ de $\Phi$ aparece con $\rho_e$ y con $\rho_q$. Unificar: $\rho_q$ = laboratorio, $\rho_e$ = propia.}%
Se define la densidad de carga propia $\rho_e$ como la contracción directa de la corriente con la cuadrivelocidad:
$$\rho_e \equiv -J^\nu u_\nu$$
la cual representa la densidad neta de carga medida por un observador que viaja solidariamente con el elemento de fluido. Simultáneamente, se define la cuadricorriente de conducción $j^\mu$ como la proyección espacial:
$$j^\mu \equiv h^\mu_{\ \nu} J^\nu$$
...
\revnota{INCONSISTENCIA con el apéndice: aquí $S^j$ y $\tau$ son \emph{solo fluido} (estilo Komissarov, con la fuerza de Lorentz como fuente), pero el apéndice usa $\mathbf S=\mathbf E\times\mathbf B+\rho hW^2\mathbf v$ y $\tau=\tfrac12(E^2+B^2)+\rho hW^2-p$ (incluye EM, estilo Miranda). Reconciliar contra lo que realmente hace el Cueva (\texttt{06\_conserved\_var.f95}).}
...
\primerparrafo En el límite de la MHD ideal ($\delta E^\mu = 0$), la relación de dispersión revela que la condición de inestabilidad no depende únicamente de la energía cinética de la cizalladura. La tensión magnética, proveniente de la perturbación del tensor de esfuerzos de Maxwell $\delta T^{\mu\nu}_{\text{em}}$, introduce un umbral de estabilización. \parencite{goedbloed-2004A}\revnota{verificar la clave: cap.\ 1 cita \texttt{goedbloed-2004} y aquí \texttt{goedbloed-2004A}; ¿misma entrada o dos distintas?} demostraron que los modos inestables son suprimidos si el producto interno espacial $(\mathbf{k} \cdot \mathbf{B}_0)^2$ es suficientemente grande, garantizando que el trabajo necesario para curvar las líneas de campo supere la energía libre del flujo.
...
\begin{equation}
    \ln \Omega'_z(t) = C + 2\gamma_{\text{KHI}}\,t.
\end{equation}
\revnota{esta constante $C$ (intercepto del ajuste log-lineal) usa el mismo símbolo que el \emph{offset} $C$ del modelo sigmoide del cap.\ de resultados; conviene renombrarla a $a$ para evitar la colisión.}
